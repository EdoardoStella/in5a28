...
class Chat{
    ...
    private $user  = 'in5a28';
    private $password   = "in5a28";
    private $database  = "in5a28";      
    ...
    public function __construct(){
        if(!$this->dbConnect){ 
            ...
            if($conn->connect_error){
                die("Fallimento collegamento con MySQL: " . $conn->connect_error);
            }else{
                $this->dbConnect = $conn;
            }
        }
	...
	public function insertChat($reciever_userid, $user_id, $chat_message) {		
		...
		if(!$result){
			return ('ERRORE- n.85: '. mysqli_error());
		} else {
	...
	public function fetchIsTypeStatus($userId){
		...
		foreach($result as $row) {
			if($row["is_typing"] == 'yes'){
				$output = ' - <small><em>Sta Scrivendo..</em></small>';
			}
		}
		return $output;
	}		
